...
import com.vh.manchester.notused.Sequence;
 ...
 * Nov 2, 2003
 */
public class ManchesterServiceImpl_Postgre implements ManchesterService, Constants {

	private static Logger log=Logger.getLogger(ManchesterServiceImpl_Postgre.class);
...
// Major functionality:
	...
	/**
	 * @see com.vh.manchester.service.manchester.ManchesterService#sendTaskEmail(Task task, HttpServletRequest request, String locale)
	 */
	public void sendTaskEmail(Task task, HttpServletRequest request, String locale) throws ServiceException {
      try {
		...
		subject.append("[HUB] ");
		subject.append(MessageUtils.getMessage(Constants_Scope.LABELS_KEY, "com.vh.email.task.subject", request, locale));
		subject.append(" "+task.getId());
		subject.append(": "+task.getTitle());
		log.debug("subject="+subject.toString());

		StringBuffer body = new StringBuffer();
		body.append("\n");
		body.append(MessageUtils.getMessage(Constants_Scope.LABELS_KEY, "com.vh.email.task.body", request, locale));
		body.append("\n\n");
		body.append(MessageUtils.getMessage(Constants_Scope.LABELS_KEY, "com.vh.email.task.ID", request, locale));
		body.append(task.getId());
		body.append("\n");
		body.append(MessageUtils.getMessage(Constants_Scope.LABELS_KEY, "com.vh.email.task.STATUS", request, locale));
		body.append(task.getTaskStatus().getDesc());
		body.append("\n");
		body.append(MessageUtils.getMessage(Constants_Scope.LABELS_KEY, "com.vh.email.task.ASSIGNED_TO", request, locale));
		body.append(task.getWho().getUsername());
		body.append("\n\n");
		body.append(MessageUtils.getMessage(Constants_Scope.LABELS_KEY, "com.vh.email.task.TITLE", request, locale));
		body.append(task.getTitle());
		body.append("\n");
		body.append(MessageUtils.getMessage(Constants_Scope.LABELS_KEY, "com.vh.email.task.SUMMARY", request, locale));
		body.append(task.getSummary());
		body.append("\n\n");
		body.append(MessageUtils.getMessage(Constants_Scope.LABELS_KEY, "com.vh.email.task.LINK", request, locale));
		body.append(task.getId());
		body.append("\n\n");
		body.append(MessageUtils.getMessage(Constants_Scope.LABELS_KEY, "com.vh.email.task.separator", request, locale));
		log.debug("body="+body.toString());
		
		/*
		String subject  =      appProp.getProperty("com.vh.email.task.subject") + task.getId();
		String body     = "\n"+appProp.getProperty("com.vh.email.task.body")    + "\n\n";

		body+=appProp.getProperty("com.vh.email.task.ID")           + task.getId()      			 +"\n";
		body+=appProp.getProperty("com.vh.email.task.STATUS")       + task.getTaskStatus().getDesc() +"\n";
		body+=appProp.getProperty("com.vh.email.task.ASSIGNED_TO")  + task.getWho().getUsername()    +"\n\n";
		body+=appProp.getProperty("com.vh.email.task.TITLE")        + task.getTitle()   			 +"\n";
		body+=appProp.getProperty("com.vh.email.task.SUMMARY")      + task.getSummary() 			 +"\n\n";
		body+=appProp.getProperty("com.vh.email.task.LINK")         + task.getId()      			 +"\n\n";
		body+=appProp.getProperty("com.vh.email.task.separator");
		*/
		sendEmail(fromName, fromAddr, toName, toAddr, subject.toString(), body.toString());
		if(!fromAddr.equalsIgnoreCase(toAddr)) sendEmail(fromName, fromAddr, fromName, fromAddr, subject.toString(), body.toString());      	
      } catch(Exception e) {
	...
	/**
	 * @see com.vh.manchester.service.manchester.ManchesterService#sendTaskUpdateEmail(Task task, HttpServletRequest request, String locale, String fromName, String fromAddr, String toName, String toAddr)
	 */
	public void sendTaskUpdateEmail(Task task, HttpServletRequest request, String locale, String fromName, String fromAddr, String toName, String toAddr) throws ServiceException {
	  try {
		StringBuffer subject = new StringBuffer();
		subject.append("[HUB] ");
		subject.append(MessageUtils.getMessage(Constants_Scope.LABELS_KEY, "com.vh.email.task.update.subject", request, locale));
		subject.append(" "+task.getId());
		subject.append(": "+task.getTitle());
		log.debug("subject="+subject.toString());
		...
		StringBuffer body = new StringBuffer();
		body.append("\n");
		body.append(MessageUtils.getMessage(Constants_Scope.LABELS_KEY, "com.vh.email.task.update.body", request, locale));
		body.append("\n\n");
		body.append(MessageUtils.getMessage(Constants_Scope.LABELS_KEY, "com.vh.email.task.ID", request, locale));
		body.append(task.getId());
		body.append("\n");
		...
		body.append(task.getSummary());
		body.append("\n");
		body.append(MessageUtils.getMessage(Constants_Scope.LABELS_KEY, "com.vh.email.task.RESOLUTION", request, locale));
		...
		/*
		String subject  =      appProp.getProperty("com.vh.email.task.update.subject") + task.getId();
		String body     = "\n"+appProp.getProperty("com.vh.email.task.update.body")    + "\n\n";
		...
		body+=appProp.getProperty("com.vh.email.task.TITLE")      + task.getTitle()   				+"\n";
		body+=appProp.getProperty("com.vh.email.task.SUMMARY")    + task.getSummary() 				+"\n";
		body+=appProp.getProperty("com.vh.email.task.RESOLUTION") + task.getComment() 				+"\n\n";
		body+=appProp.getProperty("com.vh.email.task.LINK")       + task.getId()      				+"\n\n";
		body+=appProp.getProperty("com.vh.email.task.separator");
		*/
		sendEmail(fromName, fromAddr, toName, toAddr, subject.toString(), body.toString());
	...
	/**
	 * @see com.vh.manchester.service.manchester.ManchesterService#sendEmail(String, String, String, String, String, String)
	 */
...
// Difficulty:
	...
	public Access createAccess(java.lang.String desc) throws ServiceException {
		Access ejb = null;
		try {
			Integer id = Sequence.getId("Access");
			ejb =  getAccessHome().create(id, desc);
		} catch(Exception e) {
...
// Difficulty:
	...
	public Difficulty createDifficulty(String desc) throws ServiceException {
    	Difficulty ejb = null;
    	try {
      		Integer id = Sequence.getId("Difficulty");
      		ejb =  getDifficultyHome().create(id, desc);
    	} catch(Exception e) {
...
// Locale:
	...
	public Locale createLocale(String desc, String loc) throws ServiceException {
    	Locale ejb = null;
    	try {
      		Integer id = Sequence.getId("Locale");
      		ejb =  getLocaleHome().create(id, desc, loc);
    	} catch(Exception e) {
...
// Priority:
	...
	public Priority createPriority(String desc) throws ServiceException {
    	Priority ejb = null;
    	try {
      		Integer id = Sequence.getId("Priority");
      		ejb =  getPriorityHome().create(id, desc);
    	} catch(Exception e) {
...
// Project:
	...
	*/
	public Project createProject(java.lang.String desc) throws ServiceException {
		Project ejb = null;
		try {
			Integer id = Sequence.getId("Project");
			ejb =  getProjectHome().create(id, desc);
		} catch(Exception e) {
	...
	public Integer getProjectAccess(Integer projectId, Integer userId) throws ServiceException {
		Integer accessId = new Integer(-1);		// -1=no access
		try {
...
// Task:
	...
	public Task createTask(String title, String summary, Date plannedDate, Date createDate, String comment, Integer progress, Integer difficultyId, Integer taskTypeId, Integer taskStatusId, Integer priorityId, Integer assignedToId, Integer createdById, Integer projectId) throws ServiceException {
    	...
    	try {
      		Integer id = Sequence.getId("Task");
			...
			if(projectId!=null) project = getProjectHome().findByPrimaryKey(projectId);
      		...
      		ejb = getTaskHome().create(id, title, summary, plannedDate, createDate, comment, progress, aDifficulty, aTaskType, aTaskStatus, aPriority, assignedTo, createdBy, project);
    	} catch(Exception e) {
...
// TaskStatus:
	...
	public TaskStatus createTaskStatus(String desc) throws ServiceException {
    	TaskStatus ejb = null;
    	try {
      		Integer id = Sequence.getId("TaskStatus");
      		ejb =  getTaskStatusHome().create(id, desc);
    	} catch(Exception e) {
...
// TaskType:
	...
	public TaskType createTaskType(String desc) throws ServiceException {
    	TaskType ejb = null;
    	try {
      		Integer id = Sequence.getId("TaskType");
      		ejb =  getTaskTypeHome().create(id, desc);
    	} catch(Exception e) {
	...
	public User createUser(String username, String password, String email, Timestamp lastLogin, int nofLogins, Integer localeId, Integer userTypeId, Integer lastProjectId) throws ServiceException {
    	...
    	try {
			if(checkUsername(username, null)) {		
      			Integer id = Sequence.getId("User");
				...
				if(lastProjectId!=null) aProject = getProjectHome().findByPrimaryKey(lastProjectId);
      			...
      			ejb =  getUserHome().create(id, username, password, email, lastLogin, nofLogins, aLocale, aUserType, aProject);
			} else {
...
// UserType:
	...
	public UserType createUserType(String desc) throws ServiceException {
    	UserType ejb = null;
    	try {
      		Integer id = Sequence.getId("UserType");
      		ejb =  getUserTypeHome().create(id, desc);
    	} catch(Exception e) {
